...
### 2.1 色彩計畫 (依據統一設計規範)
-   **全域一致性**: 打卡面板必須使用系統定義的 **Card Surface (`var(--color-card-light)`)**。
-   **主色調**:
...
1.  **時區處理**: 請嚴格遵守 `system_architecture.md` 第 10.1 節規範，強制使用 `Asia/Taipei` 進行計算。
2.  **防重複打卡 (Idempotency)**: Backend Action 必須檢查 `attendance` 表中是否已存在當日記錄。
3.  **鐘點制時間段**: 前端 Slot 計算需注意跨小時進位。
