...
2. **每日打卡流水帳 (Ledger)**：延遲載入 (Lazy Load) 單月打卡明細。
   - **「雙軌制與個人化排班工時」防護 (v2.4 更新)**：
       這兩類人員的「超時/不足」皆不再盲目比對 8 小時，而是精準提取該個體的「表定工作時間」(`work_start_time` ~ `work_end_time`) 作為基準。另外並加入 ±11 分鐘的誤差寬容度。
       - **月薪人員**：以「總停留時間 (Gross Time)」(不扣午休) 作為畫面上的時數。並用它去對比「表定待命時間」(`scheduledGross` = 表定下班 - 表定上班)。
       - **時薪人員**：以精算後的「淨工時 (Net Time)」(強制扣除此趟的午休) 作為畫面上的時數，並附註「已扣休 X 分」。用它去對比「表定淨工時」(`scheduledNet` = 表定下班 - 表定上班 - 午休時間)。
   - **打卡異常多重警示機制 (v2.5 追加)**：
