...
- **層次 C: 狀態視覺區分 (State Visual Distinction)**
    - 嚴格遵守《系統架構白皮書 10.2.2 節》之色彩紀律：針對「已結算」的卡片狀態，**絕對禁止**將卡片的大面積內文字色替換為高飽和的綠色系。
    - 卡片的狀態切換應僅限於：維持原始的深/淺數字文字，並輔以**(1) 卡片本體底色色階微調 (例如沈澱變灰)** 及 **(2) 右上角標誌性的綠色 Icon Badge** 來作出視覺對比。
