...
- **層次 B: 關鍵數據區 (Header Center/Right)**
    ...
    - **月薪制 (Monthly)**：顯示 `假勤/月薪`，如「假2 | $45,000」(v2.3 廢除遲到早退罰分制)。
    - **動態獎金**：顯示可編輯的 `bonus` 金額與備註。
    - **實發金額**：以最大字級顯示總計 `$Net_Amount`。
    - **工時格式**：所有個人薪資卡片中之「總工時」與「扣除午休」均採「**x小時y分**」格式顯示。 (v2.3)
- **層次 C: 狀態視覺區分**
    - 嚴格遵守《系統架構白皮書 10.2.2 節》色彩紀律。
    - 點擊卡片將開啟 **Level 2 盤查抽屜 (AuditDrawer)**。

### 4.3 盤查抽屜設計細節 (Level 2)

從右側滑出（手機板為全螢幕），運用毛玻璃質感與動畫，內含兩大區域：
1. **薪資加減法公式區**：大字體展示「基本計算 + 獎金/補貼 - 假勤扣除 = 實領總額」。未結算狀態可在此直接編輯獎金。
2. **每日打卡流水帳 (Ledger)**：延遲載入 (Lazy Load) 單月打卡明細。
   - **「雙軌制與個人化目標工時」防護與超時計算修正 (v2.5 更新)**：
       這兩類人員的「超時/不足」皆不再盲目比對 8 小時，而是精準提取該個體的「表定工作時間」(`work_start_time` ~ `work_end_time`) 作為基準。另外並加入 ±11 分鐘的誤差寬容度。
       - **超時與不足的判定基準 (Target Net Time)**：一律以 `表定總時長 - 該員工設定的固定午休 (缺設時月薪預設1H/時薪0H)` 計算出「目標淨工時」。
       - **實際算法**：以該日結算後的「實際淨工時 (Actual Net Time)」直接與「目標淨工時」相減。此舉確保當員工選擇「不放午休」時，他多做的時間會確實被計算為「超時」，而不會因為未扣午休導致目標浮動變長而誤判為「不足」。畫面上的時長顯示依然為主觀停留時間(月薪)或實際淨工時(時薪)。
   - **打卡異常多重警示機制 (v2.5 追加)**：
       - 誤差超過 11 分鐘時，顯示「超時/不足」與時間。
       - **未打下班卡**：當日有上班紀錄但無下班紀錄時，顯示橘色警示「未打下班卡」並標記異常。
...
### 5.3 薪資計算公式與請假扣除 (v2.4 法規更新)

為了符合月薪制的曆年/週年雙軌請假法規，系統執行結算時，採用**日薪固定除以 30** 與**權重積點計算**：

- **基本薪資**：月薪制即為設定之月薪；鐘點制為 `時薪 × 總工時`。
- **日薪標準 (月薪制)**：不論大/小月或 2月，日工資定額 = `Math.round(月薪 / 30)`。
- **無薪假扣除額 (Deduction)**：
  1. 系統遍歷當月所有「已批准」的請假紀錄。
  2. 依據 `utils/leave-policies.ts` 定義的權重轉換積分（事假、照顧假=1.0點；病假、生理假=0.5點；特休=0.0點）。
  3. 將總扣薪點數乘上日薪，得出最終 `deduction` 金額。
- **實發總額**：`基本薪資 + 額外獎金(Bonus) - 無薪假扣除額(Deduction)`。
