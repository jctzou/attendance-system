    ...
    return (
        <div className="space-y-4">
            ...
            <div className="block md:hidden space-y-4">
                {data.map((leave) => (
                    <div key={leave.id} className="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-slate-200 dark:border-neutral-700 p-5">
                        {/* Header: User Info */}
            ...
            <div className="hidden md:block bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-slate-200 dark:border-neutral-700 overflow-hidden">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    <thead className="bg-slate-50 dark:bg-neutral-900/50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">申請人</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">假別</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">時間 / 天數</th>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">申請時間</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">審核</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white dark:bg-neutral-800 divide-y divide-slate-200 dark:divide-slate-700">
                        {data.map((leave) => (
                            <tr key={leave.id} className="hover:bg-slate-50 dark:hover:bg-neutral-700/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-sm text-slate-600 dark:text-neutral-300 max-w-xs truncate" title={leave.reason}>
                                    {leave.reason}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-neutral-400">
