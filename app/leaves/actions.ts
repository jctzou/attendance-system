...
export async function cancelLeaveRequest(leaveId: number, cancelReason: string): Promise<ActionResult<void>> {
    return withErrorHandling(async () => {
        const input = CancelLeaveReqSchema.parse({ leaveId, cancelReason })
        const profile = await requireUserProfile()
        const supabase = await createClient()

        const { data: leave } = await supabase
            .from('leaves')
            .select('id, user_id, status, leave_type')
            .eq('id', input.leaveId)
            .single()

        if (!leave) throw { code: ErrorCodes.NOT_FOUND, message: '請假記錄不存在' }
        if (leave.user_id !== profile.id) throw { code: ErrorCodes.FORBIDDEN, message: '無權限操作' }
        if (leave.status !== 'approved') throw { code: ErrorCodes.BUSINESS_CONFLICT, message: '只能申請取消已批准的請假' }
        ...
        if (existingCancellation) throw { code: ErrorCodes.BUSINESS_CONFLICT, message: '已有待審核的取消申請' }
...
export async function getPendingCancellations() {
    return withErrorHandling(async () => {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                *,
                leave:leaves!leave_id ( start_date, end_date, leave_type, hours ),
...
export async function reviewLeaveCancellation(cancellationId: number, approved: boolean): Promise<ActionResult<void>> {
    return withErrorHandling(async () => {
        const profile = await requireUserRole(['manager', 'super_admin'])
        const supabase = await createClient()
        ...
        if (!cancellation) throw { code: ErrorCodes.NOT_FOUND, message: '取消申請不存在' }
        if (cancellation.status !== 'pending') throw { code: ErrorCodes.BUSINESS_CONFLICT, message: '此申請已被審核' }
        ...
        if (approved) {
            ...
            if (cancelledLeave && (cancelledLeave.leave_type === 'annual_leave' || cancelledLeave.leave_type === 'annual')) {
                ...
                await supabase.from('users').update({ annual_leave_used: newUsed }).eq('id', cancelledLeave.user_id)
            }
        }
